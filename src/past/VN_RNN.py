...
print(input_shift[Target].head)
input_shift[Target]=input_shift[Target]/10
print(input_shift[Target].head)
input_shift['Ventricles_Norm_PRE']=input_shift['Ventricles_Norm_PRE']/10
train_add['Ventricles_Norm_PRE']=train_add['Ventricles_Norm_PRE']/10
...
hidden1_1=Dense(10, activation='relu')(hidden1)
hidden2=Dense(len(ref.h_index),activation='relu')(hidden1_1)
hidden2_1=Dense(10,activation='relu')(hidden2)
...
preval=val*2
for i in range(1):
    if(abs(preval-val)<0.000001):
        break
    callback = [
        EarlyStopping(monitor='val_loss', patience=2, min_delta=0.0001, verbose=0)
    ]
...
test_add=pd.read_csv('test_add.csv')
test_add['Ventricles_Norm_PRE']=test_add['Ventricles_Norm_PRE']/10
evaluateRegression(test_add,test_result)
test_result.to_csv('test_predict.csv')

val_add=pd.read_csv('val_add.csv')
val_add['Ventricles_Norm_PRE']=val_add['Ventricles_Norm_PRE']/10
